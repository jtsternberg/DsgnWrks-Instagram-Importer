...
class DsgnWrksInstagram extends DsgnWrksInstagram_Debug {
	...
	protected function save_img_post() {
		...
		switch ( $this->pic->type ) {
			case 'video':
				$this->type = 'video';
				$message .= $this->upload_video_media( $p );
				break;
			...
			case 'carousel':
				...
				foreach ( $p->carousel_media as $img_item ) {
					...
					switch ( $img_item->type ) {
						...
						default:
							$sub_items[] = $this->upload_img_media( $img_item, $import['post_title'] . ' - ' . ++$index );
							break;
					}
				}

				$carousel_items = '<span class="carousel-item">'. implode( '</span><span class="carousel-item">', $sub_items ) . '</span>';

				$message = str_replace( '</li>', $carousel_items . '</li>', $message );
				break;
	...
	/**
	 * Takes an instagram media item and sideloads the vide to the currrent WordPress post
	 * @since  2.0.0
	 * @param  string|array $p            Instagram media object.
	...
	public function upload_video_media( $p, $title_append = '' ) {
		...
		foreach ( array( 'low_resolution', 'standard_resolution' ) as $size ) {
			...
			$title = sprintf( __( '%s Video', 'dsgnwrks' ), $vid_size .'x'. $vid_size ) . $title_append;
